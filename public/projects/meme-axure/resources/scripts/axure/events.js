...
//        var y = JSON.stringify(axEventObject);
    ...
    var _showCaseLinks = function(eventInfo, linksId) {
        ...
        if (left + links.clientWidth + spacing > window.innerWidth) left = window.innerWidth - links.clientWidth - spacing;
        links.style.left = left + 'px';

        var top = eventInfo.cursor.y;
        if (top + links.clientHeight + spacing > window.innerHeight) top = window.innerHeight - links.clientHeight - spacing;
